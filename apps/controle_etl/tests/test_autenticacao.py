...
class TestAutenticacaoApiKey:
    ...
    def _drf_request(self, factory, headers: dict | None = None):
        rf_request = factory.get("/", **(headers or {}))
        return Request(rf_request)

    def test_sem_header_retorna_none(self, factory, auth, settings):
    ...
    def test_chave_correta_autentica(self, factory, auth, settings):
    ...
    def test_chave_incorreta_lanca_excecao(self, factory, auth, settings):
    ...
    def test_header_com_hifens_e_normalizado(self, factory, auth, settings):
